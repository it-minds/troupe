...
Plus [`charts/troupe`](charts/troupe), which deploys the lot. On a machine, from the same
release: `troupe` and `troupe-daemon` for Linux, macOS and Windows, which `install.sh` and
`install.ps1` install, and the desktop app's installers.
