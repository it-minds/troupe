defmodule Troupe.Config.JSONC do
  ...
  defp do_strip(<<"\\", c, rest::binary>>, :string, acc), do: do_strip(rest, :string, [<<"\\", c>> | acc])
  defp do_strip(<<"\"", rest::binary>>, :string, acc), do: do_strip(rest, :code, ["\"" | acc])
  defp do_strip(<<c::utf8, rest::binary>>, :string, acc), do: do_strip(rest, :string, [<<c::utf8>> | acc])
  ...
  defp do_strip(<<c::utf8, rest::binary>>, :code, acc), do: do_strip(rest, :code, [<<c::utf8>> | acc])

  defp do_strip(<<"\n", rest::binary>>, :line_comment, acc), do: do_strip(rest, :code, ["\n" | acc])
  defp do_strip(<<_::utf8, rest::binary>>, :line_comment, acc), do: do_strip(rest, :line_comment, acc)

  defp do_strip(<<"*/", rest::binary>>, :block_comment, acc), do: do_strip(rest, :code, acc)
  defp do_strip(<<_::utf8, rest::binary>>, :block_comment, acc), do: do_strip(rest, :block_comment, acc)
