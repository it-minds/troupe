defmodule Troupe.Memory do
  ...
  Pure. `Troupe.Session.Memory` owns the file; nothing here touches disk. The brief is
  durable but never authoritative: it is not an event, replay ignores it, and
  `list_files`/`grep`/`read_file` remain the truth about current contents.

  Parsing is lossless. Sections keep their order, headings nobody recognises are
  preserved, and text before the first heading round-trips as a titleless section, so
  hand edits survive a librarian refresh.
  """
  ...
  @max_chars 6_000
  @max_age_days 7
  # The tracked-file count must drift by both this fraction and this many files before
  # the brief is called stale.
  @drift 0.10
  @min_drift 10
  ...
  def render(%__MODULE__{} = m), do: frontmatter(m) <> body(m.sections) <> "\n"

  @doc """
  The `# Project brief` block for the system prompt, or `""` when there is nothing to
  say. `:max_chars` caps it (default #{@max_chars}).
  """
  ...
  def add_note(%__MODULE__{} = m, agent, text) do
    text = squish(text)
    line = "- #{Date.utc_today()} #{agent}: #{text}"
  ...
  @doc "The section titles, in order, without the titleless preamble."
  ...
  HEAD is recorded for a human to read but deliberately does not trigger a refresh:
  every commit would.
  """
  ...
  # Both a relative and an absolute floor: without the floor a handful of new files makes
  # a small repository's brief permanently stale, and the brief's own first write would
  # invalidate it.
