...
- `lib/troupe/ui/tui/*` Model (fold over events, rebuildable), View (widgets), Server (`ExRatatui.App`); `ui/headless/printer.ex`; `cli.ex`, `cli/runner.ex` (Burrito entry, blocks in the UI supervisor)
- `native/reaper/reaper.zig` + `Mix.Tasks.Compile.Reaper` in `mix.exs`: every OS process runs under reaper
- `test/support/helpers.ex` (`start_session!`, `await_state`, `eventually`), `test/support/tui_helpers.ex` (headless TUI on `CellSession`, `screen_text`, `press`)
