...
| an `ask_user` question shows as a menu and the answer goes back as `question.answer` | done (Decision 106; troupe-remote Decision 651) | `question_client_test.exs`, `remote_translate_test.exs` "a question…" |
| `/mcp` lists the workspace's own MCP servers from the daemon | done (Decision 107; troupe-remote Decision 654) | `phase3_client_test.exs` "the /mcp page…" |
| a near budget limit shows as a warning once; `--full-send` turns it off | done (Decision 107; troupe-remote Decision 655) | `phase3_client_test.exs` "a limit that is near…", `remote_translate_test.exs` "a budget warning…" |
| `read_output`, `read_roots`, `glob`, `git_read`, `web_fetch`, the eleven definitions | in the harness the pin points at; nothing to do here beyond the pin | troupe-remote #18–#24 |

Rows of the plan's phase 3 table are now all either here or in the harness. What the
plan listed and this repository dropped in phase 2 is back where it belongs: in the core,
behind the protocol, proven by the core's tests and these.
