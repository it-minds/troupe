defmodule Troupe.LLM.Catalog do
  @moduledoc """
  What a provider says about its own models: context window, output cap and
  price. Pure — `Troupe.LLM.Catalog.Store` fetches and owns the cache file;
  nothing here touches disk or the network.

  Three response shapes are understood, because no two providers agree and only
  one of them prices anything:

    * `:anthropic` — `GET /v1/models` gives `max_input_tokens` and `max_tokens`
      per model. There is no pricing endpoint, so those entries carry no price.
    * `:litellm` — a LiteLLM proxy's `GET /model_group/info` gives windows *and*
      per-token cost, keyed by model group, which is the name you address. This
      is the one source that has prices.
  ...
  Entries never override an explicit `windows:` entry in config.yaml — the
  catalog fills gaps and prices, it does not overrule what the user wrote.
  """
  ...
  @doc """
  Parses one provider's model listing. Unknown or malformed entries are dropped
  rather than crashing a refresh: a gateway that serves one broken row should
  still contribute the other nineteen.
  """
  ...
  # `mode` separates chat models from embeddings and transcription, and the
  # wildcard group LiteLLM synthesises has null limits — neither is addressable
  # as an agent's model.
  ...
  def priced?(%__MODULE__{input: input, output: output}),
    do: is_float(input) and is_float(output)

  @doc """
  Price per million tokens, as it reads in a menu: `$0.25/$1.50`, input then
  output. `nil` when the provider quoted no price.
  """
  ...
  @doc """
  What one response cost, in dollars, or `nil` for an unpriced model. Each of
  the four token classes bills at its own rate: a cache read is a fraction of
  fresh input and a cache write a premium on it, so pricing a long session off
  the input rate alone would be wrong by more than it is right. Providers that
  quote no cache rates are charging the input rate for cache reads and writes,
  which is what falling back to it means.
  """
