...
Call `finish` with the answer in the summary: a direct reply first, then the file paths and line numbers it rests on. If the question cannot be answered without a change to the code, say so and stop — the user will write `AI!` for that.
