...
| [entra.md](entra.md) | **Microsoft Entra, end to end**: which claim is the person and why it cannot be changed later, the app registration and the three ways a groups claim goes quietly wrong, making a group into a team, turning SCIM on, the provisioning app's mappings, and exactly what a deprovision does |
...
4. **Platform admin comes from an identity-provider group**, read from a token claim — never from a scope. Ask for `groups` as a scope and Entra refuses every sign-in ([roles-and-permissions.md §2](roles-and-permissions.md#2-the-three-admin-roles); [integrations.md §1](integrations.md#1-identity-provider-oidc)). **And settle `plane.subjectClaim` before the first sign-in** — `oid` on Entra if you will ever run SCIM, because Entra's `sub` is per-registration and is not something SCIM can push ([entra.md §0](entra.md)).
