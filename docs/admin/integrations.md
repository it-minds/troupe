...
| A SPA/loopback redirect for the GUI and MCP clients | the GUI and an OAuth-capable MCP client authenticate in the browser | `docs/plans/admin-surface.md:87-90`; `clients/gui` |
...
The browser client is `clients/gui`, and the chart serves it at `gui.basePath` (`/app`) on the plane's own host (Decision 670) — same origin as the plane, so the chart's own GUI needs no CORS entry. What a GUI served from anywhere else needs from this deployment (`clients/gui/docs/AUDIT.md:85`): its origin in `plane.corsOrigins` → `TROUPE_CORS_ORIGINS` (exact origin, comma-separated; `runtime.exs:265-270`; `web/cors.ex`), its origin in `operator.workerAllowedOrigins` → `TROUPE_ALLOWED_ORIGINS` on every worker pod (`resources.ex:589-593`), a SPA redirect registered at the identity provider. The chart's GUI listens on 8080 behind the `troupe-gui` Service. The plane's CORS plug answers only two-segment `/.well-known/<doc>` paths, not `/.well-known/oauth-protected-resource/mcp` (plane audit note 14). Its admin guide is [../../clients/gui/docs/admin/README.md](../../clients/gui/docs/admin/README.md).
