...
| `llm.smallModel` | string | — | injected as `TROUPE_SMALL_MODEL` but read by nothing ([AUDIT.md §3.3](../history/AUDIT.md)) | 56-58 |
...
`"image": "release"` means the worker image of the release the plane is running: `TROUPE_WORKER_IMAGE`, which the chart sets from `worker.image.repository` and `worker.image.tag` (empty means the chart's `appVersion`). The row keeps the word and the plane resolves it whenever it renders the `WorkerProfile`, so the policy check sees the resolved image exactly as it would a typed one; a plane deployed without a worker image refuses `release` on `admin.profile.put`. When a plane starts, every profile whose image is `release` and whose `WorkerProfile` carries a different image is written again the way an edit is — applied in direct mode, committed in GitOps mode, where "carries" means the last committed manifest — and audited as `profile.put` by `system:release`, with the image's `from` and `to`. A cluster that does not answer is tried again, from 30 seconds backing off to every 10 minutes, without holding up the plane's start (`fleet/release_image.ex`). The pods themselves still move only when they are recreated (§4). The console editor shows what `release` resolves to under the image field. Use it for profiles that should track the platform, and a tag or digest for one that should not move. The policy has to allow `worker.image.repository`: `values.small.yaml` and `values.scaleway.yaml` point `allowedImageRepositories` at their own registry, so set `worker.image.repository` to match there.
