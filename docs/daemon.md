...
| catalog, usage, headroom | the price catalog and the usage fold were already here; `Troupe.Agent.Headroom` adds the five fractions to `agent_state.budget.headroom` and a `budget_warning` at `budget_warn_at`, off with `full_send` (Decision 655) | `headroom_test.exs` |
...
| the reasoning round-trip | `Troupe.LLM.Reasoning` is a content block, captured off both streams, logged, replayed only to the provider that made it (Anthropic only with thinking on); `llm_delta` `kind: "reasoning"`; ACP `agent_thought_chunk` (Decision 658) | `reasoning_test.exs`, `reasoning_shape_test.exs`, `providers_test.exs` "captures thinking, its signature and redacted thinking…", "with thinking off…", "accumulates reasoning_content…", "the `reasoning` spelling is accepted…" |
| output caps for reasoning models | a model's `reasoning_effort` reaches the request through `Config.target/2`; OpenAI sends `max_completion_tokens` + `reasoning_effort` and answers a 400 naming the other field by asking again with it; Anthropic turns the level into `thinking.budget_tokens` and raises `max_tokens` to hold it (Decision 658) | `providers_test.exs` "a reasoning effort asks for max_completion_tokens…", "a 400 that asks for max_completion_tokens by name…" |
| truncation recovery | `handle_response/2` looks at the reply first: a `max_tokens` stop is asked again once with a note (`truncated`, `user_input` from `harness`) and then ends `output_truncated`; a tool call cut mid-argument is answered with an error, not run; an empty reply is nudged once and then ends `empty_reply`; a `refusal` ends `refused`; a subagent hands its parent what there was (Decision 659) | `limits_test.exs` "the output cap", "an empty reply" |
| context overflow and error classes | `Provider.classify/1` and `describe_error/1`; a 400 for a prompt too long compacts once (`compacted.reason: context_overflow`) and sends the turn again, a second one or a conversation too short to compact fails with a line that says what to do; `llm_error.reason` is a sentence; a 429 gets more attempts and waits `retry-after` (Decision 659) | `limits_test.exs` "context overflow", `provider_errors_test.exs` |
| the budget question | a spent budget asks through `Troupe.Session.Questions` (`question_asked` with `call_id` `budget-<n>`, options `allow` / `always` / `deny`, answered with `question.answer`); the agent waits in state `waiting`; `allow` grants the original allowance again and warns afresh, `always` lifts the subtree's budget, `deny` ends `budget_exhausted`; `budget_ask_started` / `budget_ask_answered` are folded; `budget_asks: false` (the plane's terms) keeps the stop, a subagent reports to its parent instead, `full_send` never asks, `approvals: deny` answers no itself (Decision 660) | `budget_question_test.exs`, `resilience_test.exs` "budgets", `delegation_test.exs` "a subagent that runs out of budget" |
