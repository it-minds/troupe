...
```
themes/
  footlight.tokens.json   Footlight kit.dc.html    ← default
...
1. **Footlight is preselected.** Someone who presses Continue without reading gets the default and loses nothing.
...
Skipping is allowed, and skipping means Footlight with system mode.

### Afterwards

**Settings → Appearance**, same three cards, same live previews, plus the mode control. Changing a theme applies immediately, with no reload and no confirmation — it is a `data-theme` attribute on the document root and nothing more.
...
4. **Footlight stays the default.** It is the least opinionated and the closest to the platform's existing drawn identity. Limelight is the one to promote if the client wants Troupe to look like nothing else; Signal if they want it to look current.
