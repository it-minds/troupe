...
| a kubeconfig (`KUBECONFIG` or `~/.kube/config`, context from `TROUPE_KUBE_CONTEXT`) with the Troupe CRDs installed | `apps/troupe_operator/test/troupe/operator/{cluster,admin_cluster}_test.exs` | `setup_all` prints `SKIPPED` and returns `:ok`; the tests pattern-match `%{conn: conn, suffix: suffix}` in their arguments and error rather than skip. They are tagged `@tag :cluster` but nothing excludes that tag. Unconfirmed whether CI's `check` job, which has no cluster, is green with these present; see [../AUDIT.md](../AUDIT.md) §3.14 and open question 2 | `cluster_case.ex:25-54,112-119`; `cluster_test.exs:39-41` |
| a cluster with `ghcr.io/objective-mj/troupe-worker:dev` loadable | `apps/troupe_operator/test/troupe/operator/latency_cluster_test.exs` | prints `SKIPPED` with `scripts/kind-up` and `scripts/build-images troupe_worker` | `latency_cluster_test.exs:21-22,55-72` |
| a real API server (for `TokenReview`) | `apps/troupe_plane/test/troupe/plane/enrolment_test.exs` | `setup_all` prints `SKIPPED`; `requires_cluster/1` flunks | `enrolment_test.exs:20-40,146-147,170-190` |
